...
class NoFrixionMoneyMoovModelsPaymentRequestMinimal implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['payment_methods']) && !in_array($this->container['payment_methods'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'payment_methods', must be one of '%s'",
     ...
     *
     * @param string|null $payment_methods The payment methods that the payment request supports. When setting using form data  should be supplied as a comma separated list, for example \"card, pisp, lightning, applePay\".
     *
     * @return self
     */
    public function setPaymentMethods($payment_methods)
    {
        ...
        $allowedValues = $this->getPaymentMethodsAllowableValues();
        if (!in_array($payment_methods, $allowedValues, true)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value '%s' for 'payment_methods', must be one of '%s'",
                    $payment_methods,
